...
function init() {
  ...
  function drawText() {
    ...
    ctx.fillText("Ngủ Ngon nhóoo", width / 2, height / 2.2 + 400);
  }
  ...
  for (var i = 0; i < Math.PI * 2; i += dr)
    pointsOrigin.push(scaleAndTranslate(heartPosition(i), 60, 4, 0, 0));
  for (var i = 0; i < Math.PI * 2; i += dr)
    pointsOrigin.push(scaleAndTranslate(heartPosition(i), 48, 3, 0, 0));
  for (var i = 0; i < Math.PI * 2; i += dr)
    pointsOrigin.push(scaleAndTranslate(heartPosition(i), 36, 2, 0, 0));
